...
<main class="flex-1 p-10">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow p-8 mb-10">
            <form action="${pageContext.request.contextPath}/estudiante/buscar-tutor" method="GET" id="formBusqueda" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="carrera" class="w-full rounded-xl border border-slate-200 p-3 focus:ring-2 focus:ring-primary">
                            ...
                            <c:forEach var="car" items="${carreras}">
                                <option value="${car.name()}" <c:if test="${car.name() == carreraSeleccionada}">selected</c:if>>
                    ...
                    <div>
                        ...
                        <select name="materia" class="w-full rounded-xl border border-slate-200 p-3 focus:ring-2 focus:ring-primary">
                            ...
                            <c:forEach var="mat" items="${materias}">
                                <option value="${mat.codigo}" <c:if test="${mat.codigo == materiaSeleccionada}">selected</c:if>>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <c:forEach var="tutor" items="${tutores}">
                                <div class="bg-white rounded-xl shadow hover:shadow-lg transition-shadow p-6">
                                    <div class="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-600">
                                                <c:out value="${tutor.carrera != null ? tutor.carrera.nombre : 'Sin carrera'}"/>
                                            </p>
                                        </div>
                                    </div>
                                        ...
                                        <div class="flex flex-wrap gap-2 mt-2">
                                            <c:forEach var="mat" items="${tutor.codigosMateriaRelacionadas}" varStatus="st">
                                                <c:if test="${st.count <= 2}">
                                                    <span class="text-xs bg-indigo-50 text-primary px-2 py-1 rounded font-mono">
                                                        <c:out value="${mat}"/>
                                                    </span>
                                                </c:if>
                                            </c:forEach>
                                    ...
                                    <a href="${pageContext.request.contextPath}/estudiante/tutor/detalle?id=${tutor.id}"
