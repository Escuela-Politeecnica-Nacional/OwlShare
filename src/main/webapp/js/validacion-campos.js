...
(function (global) {
    ...
    function validarComentarioMentoria(valor) {
        if (!valor || !valor.trim()) {
            return 'Debes describir tu duda o motivo.';
        }
        var limpio = valor.trim();
        if (limpio.length > LIMITES.comentarioMentoriaMax) {
            return 'El mensaje no puede superar ' + LIMITES.comentarioMentoriaMax + ' caracteres.';
        }
        if (!PATRONES.comentarioMentoria.test(limpio)) {
            return 'El mensaje contiene caracteres no permitidos.';
        }
        return null;
    }

    function inicializarContadorTextarea(textarea, contadorEl, max) {
        if (!textarea || !contadorEl) {
            return;
        }

        function actualizar() {
            var longitud = textarea.value.length;
            contadorEl.textContent = longitud + ' / ' + max;
            contadorEl.classList.toggle('text-red-600', longitud > max);
            contadorEl.classList.toggle('text-slate-500', longitud <= max);
        }
    ...
    global.OwlValidacion = {
        ...
        inicializarContadorTextarea: inicializarContadorTextarea
    };
})(window);
