...
/**
 * Catálogo de materias por carrera según mallas curriculares FIS-EPN.
 * Usa los enums {@link Carrera} y {@link Semestre}.
 */
public final class MateriasCatalogo {

    private static final Map<Carrera, List<Materia>> POR_CARRERA = new EnumMap<>(Carrera.class);

    /**
     * Materias excluidas de tutoría: prácticas, titulación e integración curricular.
     */
    private static final Set<String> CODIGOS_NO_TUTORIZABLES = Set.of(
            "PSCD202",
            "PRLD105",
            "TITD104",
            "TITD201",
    ...
    static {
        POR_CARRERA.put(Carrera.SOFTWARE, List.of(
        ...
        POR_CARRERA.put(Carrera.COMPUTACION, List.of(
                mat("MATD113", "Álgebra Lineal", 1),
                mat("MATD123", "Cálculo en una Variable", 1),
                mat("FISD134", "Mecánica Newtoniana", 1),
                mat("ICCD144", "Programación I", 1),
        ...
        POR_CARRERA.put(Carrera.CIENCIA_DATOS, List.of(
                mat("MATD113", "Álgebra Lineal", 1),
        ...
        POR_CARRERA.put(Carrera.SISTEMAS_INFORMACION, List.of(
                mat("MATD113", "Álgebra Lineal", 1),
                mat("MATD123", "Cálculo de una Variable", 1),
                mat("FISD134", "Mecánica Newtoniana", 1),
                mat("ICCD144", "Programación I", 1),
    ...
    public static List<Materia> materiasDeCarrera(Carrera carrera) {
        ...
        return POR_CARRERA.getOrDefault(carrera, List.of());
    }

    public static Materia buscarEnCarrera(Carrera carrera, String codigo) {
        if (carrera == null || codigo == null || codigo.isBlank()) {
            return null;
        }
    ...
    public static Materia buscarPorCodigo(String codigo) {
    ...
    public static List<Materia> materiasPorSemestre(Carrera carrera, int numeroSemestre) {
        if (carrera == null || numeroSemestre < 1) {
            return List.of();
        }
        return materiasDeCarrera(carrera).stream()
                .filter(m -> m.getSemestre() == numeroSemestre)
                .toList();
    }

    public static boolean esMateriaTutorable(Materia materia) {
        if (materia == null) {
            return false;
        }
        if (CODIGOS_NO_TUTORIZABLES.contains(materia.getCodigo().toUpperCase(Locale.ROOT))) {
            return false;
        }
        String nombre = materia.getNombre().toLowerCase(Locale.ROOT);
        return !nombre.contains("prácticas")
                && !nombre.contains("integración curricular")
                && !nombre.contains("examen complexivo");
    }

    public static boolean esMateriaTutorable(String codigo) {
        Materia materia = buscarPorCodigo(codigo);
        return materia != null && esMateriaTutorable(materia);
    }
    ...
    public static String materiasPorCarreraJson() {
        ...
        for (int i = 0; i < carreras.length; i++) {
            ...
            List<Materia> materias = POR_CARRERA.get(carrera).stream()
                    .filter(MateriasCatalogo::esMateriaTutorable)
                    .toList();
